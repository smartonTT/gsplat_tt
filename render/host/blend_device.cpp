...
inline bool l1_masks_enabled() { return false; }  // L1_MASKS dropped
...
inline bool payload_enabled() { return false; }  // BLEND_PAYLOAD dropped
...
static void build_program_and_workload_mb(DeviceContext& ctx) {
    ...
    // Production blend-kernel configuration (baked): RESIDENT_BLEND + SFPU_CULL +
    // TILE_BUCKET + BLEND_AOS, no soft-float MB_DEVCULL. The reader reads the
    // precomputed SFPU mask, fetches each candidate as one 64B AoS record, and
    // serves each tile from its dense L1-resident bucket; the compute derives the
    // conic on the SFPU (MB_DEVCONIC).
    const bool resident_blend = true;     // RESIDENT_BLEND=1
    const bool dev_cull = false;          // MB_DEVCULL dropped (SFPU cull path)
    const bool dev_conic = true;          // SFPU derives conic from raw cov
    const bool sfpu_cull = true;          // SFPU_CULL=1
    const bool resident_reader = true;    // resident_blend && sfpu_cull
...
static double process_frame_mb_devcull_resident(
    ...
    const bool payload = false;       // BLEND_PAYLOAD dropped
    const bool sfpu_cull = true;      // SFPU_CULL=1
    const bool blend_aos = true;      // BLEND_AOS default-on
    const bool tile_bucket = true;    // TILE_BUCKET=1
...
static double process_frame_resident(
    ...
    const bool blend_aos = true;  // BLEND_AOS default-on (production)
    uint32_t blendrec_addr = 0;
    if (blend_aos) {
