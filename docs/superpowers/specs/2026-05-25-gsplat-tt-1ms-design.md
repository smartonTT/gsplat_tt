...
- **No visible artifacts is the primary gate, not a hard PSNR floor.** Validator must visually inspect every render + diff×10 image for tile seams, banded patterns, missing-splat holes, color clipping, ringing, NaN sentinels, geometry shift. PSNR is a numeric proxy used per optimization class (>100 dB for kernel-algebra, >50 dB for binning/sort/host-prep). The prior run failed because PSNR-only acceptance let tile artifacts through.
...
Two layered changes, evaluated independently then composed. Each must hold the primary gate (no visible artifacts) and the class PSNR floor (>100 dB for kernel-algebra).
...
|---|---|---|
| kernel-algebra (Layer A/B) | >100 dB any view | NEEDS_REVIEW |
| precompute (view-invariant host work) | >100 dB | NEEDS_REVIEW |
| dispatch (skip-tile, launch order, overlap) | >100 dB | NEEDS_REVIEW |
| binning (tighter bbox, etc.) | >50 dB | KEEP if visuals pass; below 50 → NEEDS_REVIEW |
| sort (reorder accumulation) | >50 dB | KEEP if visuals pass; below 50 → NEEDS_REVIEW |
| host-prep (other view-dependent host work) | >50 dB | KEEP if visuals pass; below 50 → NEEDS_REVIEW |
...
- `graph-psnr.png` — y=min PSNR across views; dashed at 100 dB and 50 dB floors.
