...
- Date: 2026-05-26
- Status: REJECTED (kernel hang reproduces with clean cache + clean device)
...
**Hang reproduces deterministically.** Two attempts: first run hung at cycle 1
on a device that may have had residual state from iter-012. Second run after
explicit JIT cache wipe + verified clean device (lsof clean, only dev viewer
process at PID 200598 idle) — also hung at cycle 1 within 5 minutes.

**Empirical lesson:** removing `binary_dest_reuse<ELWMUL, DEST_TO_SRCA>(CB_SAT_MASK)`
from a fused acquire that contains `mul_tiles → binary_dest_reuse → pack_tile`
is not freely reversible. The packer appears to depend on internal pipeline
state set up by the `binary_dest_reuse_tiles_init` call, even though it
nominally writes from dst[0] (which holds the mul_tiles result). Without that
init, pack stalls and the kernel deadlocks.

This is the **inverse** of iter-007/iter-010's PSNR cost: adding a
binary_dest_reuse costs ~1 dB but is FREE in latency. *Removing* one is not
free — it costs the entire kernel.

**Lesson for future iters:** treat the `mul_tiles → binary_dest_reuse → pack`
triad as atomic. To recover PSNR headroom we must remove the *triad* (revert
the iter-007/iter-010 fusions to a 2-acquire form), not just the
binary_dest_reuse half of it.
