...
### Active baseline for new iterations: post-iter-034 (`smarton/opt-stable`, with `GSPLAT_TT_USE_SHM=1`, `GSPLAT_TT_MAX_G_PER_TILE=1024` (default), iter 034 cull active in `Backend.cull_pairs`, `OMP_NUM_THREADS=4`/`MKL_NUM_THREADS=4`/`OPENBLAS_NUM_THREADS=4` set in bench env)
  **Kernel 41.5 ms**, total 102.6 ms at 1024×1024 stitch_doll. PSNR 41.8 dB (clean-keep). entries 752k.
  **User target = 1 ms kernel.** Currently 41.5 ms → need ~42× more. Path forward (revised): (a) iter 035: tighter Mahalanobis eps + faster cull (eliminate the 20 ms host cost, e.g. inline into get_tile_assignments via vectorized cov-inv); (b) iter 036: kernel block-wide early term using actual per-pixel T (vs my host-side peak-alpha bound from iter 033); (c) iter 037: 4-face split (each tile has 4 16×16 face Gaussian lists); (d) iter 038: FPU refactor retry; (e) iter 039: persistent kernel.
