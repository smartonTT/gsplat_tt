...
### Active baseline for new iterations: post-iter-032 (`smarton/opt-stable`, with `GSPLAT_TT_USE_SHM=1`, `GSPLAT_TT_MAX_G_PER_TILE=1024` (default), `OMP_NUM_THREADS=4`/`MKL_NUM_THREADS=4`/`OPENBLAS_NUM_THREADS=4` set in bench env)
  **Kernel 47.4 ms**, total 88.3 ms at 1024×1024 stitch_doll. PSNR 38.9 dB (clean-keep gate).
  At 480×640: kernel 19.5 ms, total 60.2 ms, PSNR 30.6 dB (needs-review).
  **User target = 1 ms kernel.** Currently 47.4 ms → need 47× more. Path forward: (a) prefix-T saturation cull (replaces fixed cap, adaptive per tile, expected to be strictly better); (b) block-wide early term in kernel; (c) FPU refactor (retry with different API); (d) Dst-resident accumulators; (e) persistent kernel.
