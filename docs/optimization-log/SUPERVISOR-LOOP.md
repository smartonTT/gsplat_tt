...
### Active baseline for new iterations: post-iter-035 (`smarton/opt-stable`, with `GSPLAT_TT_USE_SHM=1`, `GSPLAT_TT_MAX_G_PER_TILE=1024` (default), iter 034 cull active w/ `eps=5e-2` (iter 035 default), `OMP_NUM_THREADS=4`/`MKL_NUM_THREADS=4`/`OPENBLAS_NUM_THREADS=4` set in bench env)
  **Kernel 23.3 ms**, total 81.0 ms at 1024×1024 stitch_doll. PSNR 37.8 dB (clean-keep). entries 344k.
  **User target = 1 ms kernel.** Currently 23.3 ms → need ~23× more. **Cumulative since iter 029 baseline (106.7 ms): −78% kernel reduction.** Path forward: (a) iter 036: kernel block-wide early term; (b) iter 037: 4-face split; (c) iter 038: FPU refactor v2; (d) iter 039: persistent kernel + dispatch overlap; (e) iter 040: faster cull (inline Σ⁻¹ at projection time → eliminate 20 ms host cost; matters once kernel < cull).
