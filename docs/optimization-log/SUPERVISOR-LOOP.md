...
### Active baseline for new iterations: post-iter-030 (`smarton/opt-stable`, with `GSPLAT_TT_USE_SHM=1` and `OMP_NUM_THREADS=4`/`MKL_NUM_THREADS=4`/`OPENBLAS_NUM_THREADS=4` set in bench env)
  **Kernel 68.4 ms**, total 178.8 ms, prep 12.2 ms, sort 34.4 ms, project 102.0 ms (long pole), save_npy 17.3 ms, load_npy 3.6 ms, daemon_rt ~0 ms at 1024×1024 stitch_doll. Visible 277,825 / entries 1,014,180.
  At 480×640: kernel 45.7 ms, total 107.9 ms, project 68.0 ms, sort 19.9 ms, prep 7.0 ms. Visible 253,507 / entries 606,329.
  **User target = 1 ms kernel.** Currently 68.4 ms → need ~68× more reduction. Path: FPU refactor (3-4×) + 4-face split (~30%) + early-term + dst-resident state + persistent kernel.
