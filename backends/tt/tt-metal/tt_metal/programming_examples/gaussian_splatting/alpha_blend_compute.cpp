    ...
    for (uint32_t t = 0; t < num_tiles; t++) {
        ...
        for (uint32_t g = 0; g < g_count; g++) {
            ...
            if (!block_dead && g > 0 && (g & 0xFu) == 0u) {
                ...
                reduce_init<PoolType::MAX, ReduceDim::REDUCE_SCALAR>(CB_SAT_MASK, CB_CONST_ONE, CB_T_TMP);
                tile_regs_acquire();
                reduce_tile<PoolType::MAX, ReduceDim::REDUCE_SCALAR>(CB_SAT_MASK, CB_CONST_ONE, 0, 0, 0);
                tile_regs_commit();
                tile_regs_wait();
                ...
                uint32_t sat_max_bits = ckernel::read_tile_value(CB_T_TMP, 0, 0);
                cb_pop_front(CB_T_TMP, 1);
                float sat_max;
                __builtin_memcpy(&sat_max, &sat_max_bits, sizeof(float));
                if (sat_max < 0.5f) {
                    block_dead = true;
                }
            }
