...
void kernel_main() {
    ...
    for (uint32_t t = 0; t < num_tiles; t++) {
        ...
        for (uint32_t g = 0; g < g_count; g++) {
            ...
            // iter-097: dropped the 3 binary_dest_reuse<ELWADD,DEST_TO_SRCA>
            // ops on R/G/B_STATE. Instead, the packer's L1 accumulate mode
            // (enabled below before the R/G/B pack_tile calls) adds dst into
            // the existing state-CB tile. With fp32 state CBs (iter-097 host
            // change), the L1 add is fp32+fp32 — equal precision to the
            // binary_dest_reuse path it replaces, but saves 3 SFPU ops + 1
            // shared init per Gaussian. T_state uses ELWSUB (no L1 acc path),
            // so its binary_dest_reuse stays; L1 acc is disabled before T pack.
            ...
            cb_reserve_back(CB_COLOR_B_STATE, 1);
            cb_reserve_back(CB_T_STATE, 1);
            // iter-097: L1 accumulate ON for R/G/B (state += dst).
