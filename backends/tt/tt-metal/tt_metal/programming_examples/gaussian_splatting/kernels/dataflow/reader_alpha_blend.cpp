...
void kernel_main() {
    ...
    const uint32_t scratch_static_addr = reinterpret_cast<uint32_t>(scratch_static);
    const uint32_t scratch_gids_addr = reinterpret_cast<uint32_t>(scratch_gids);
    ...
    for (uint32_t t = 0; t < tile_ids_count; t++) {
        ...
        for (uint32_t g = 0; g < g_count; g++) {
            ...
            if (gid_page != last_gid_page) {
                ...
                noc_async_read(gids_noc, scratch_gids_addr, gids_page_bytes);
                noc_async_read_barrier();
                last_gid_page = gid_page;
            }
            uint32_t gid = scratch_gids[entry_id % gids_per_page];
            ...
            noc_async_read(dyn_noc, scratch_dyn_addr, dyn_pack_page_bytes);
            noc_async_read(static_noc, scratch_static_addr, static_page_bytes);
            noc_async_read_barrier();
            ...
            for (uint32_t i = 0; i < 4; i++) {
                out[5 + i] = scratch_static[i];
            }
            for (uint32_t i = 9; i < scalar_pack_page_bytes / 4; i++) {
                out[i] = 0;
            }
