...
//   7: m2thr_addr (fp32)  8: opacok_addr (int32)
//   9: keep_addr (int32 output)
//  10: page_start  11: page_count  12: P
//  13: tiles_x  14: tile_size
//
// COMPILE-TIME ARGS: 10 TensorAccessorArgs in the above order.
...
void kernel_main() {
    ...
    const uint32_t opacok_addr = get_arg_val<uint32_t>(8);
    const uint32_t keep_addr   = get_arg_val<uint32_t>(9);
    const uint32_t page_start  = get_arg_val<uint32_t>(10);
    const uint32_t page_count  = get_arg_val<uint32_t>(11);
    const int P                = static_cast<int>(get_arg_val<uint32_t>(12));
    const int tiles_x          = static_cast<int>(get_arg_val<uint32_t>(13));
    const float tsf            = static_cast<float>(get_arg_val<uint32_t>(14));
    ...
    constexpr auto opacok_args = TensorAccessorArgs<m2thr_args.next_compile_time_args_offset()>();
    constexpr auto keep_args   = TensorAccessorArgs<opacok_args.next_compile_time_args_offset()>();
    ...
    const auto opacok_acc = TensorAccessor(opacok_args, opacok_addr, PAGE_BYTES);
    ...
    constexpr uint32_t CB_OK   = 8;
    constexpr uint32_t CB_KEEP = 9;
    ...
    const uint32_t ok_l1   = get_write_ptr(CB_OK);
    ...
    auto okp  = reinterpret_cast<volatile int32_t*>(ok_l1);
    ...
    auto load_attrs = [&](int g) {
        ...
        if (pg != attr_cached_page) {
            ...
            noc_async_read(get_noc_addr(static_cast<uint32_t>(pg), opacok_acc), ok_l1,  PAGE_BYTES);
